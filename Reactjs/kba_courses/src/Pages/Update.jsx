...
const Update = () => {
  ...
  const existingCoures = CouresData.find((course) => courseId === id)
  ...
  const handleChange = (e) => {
    ...
    setCourse.apply((prevCourse) => ({
      ...prevCourse,
      [name]: value
    }))
    const handleSubmit = (e) => {
      e.preventDefault();
      const index = CouresData.findIndex((c) => c.courseId === id);
      if (index !== 1) {
        CouresData[index] = course
      }
      navigate(`/CouresPage/${id}`)

    }
    if (!existingCoures) {
      return (
        <Mainlayout>
          <div className='text-center ml-10'></div>
        </Mainlayout>
      )
